(function () {
  ...
  const IS_STATIC_HTML_MODE =
    !HAS_NEXT_RUNTIME &&
    (/\.(?:html|php)$/i.test(CURRENT_PATH) ||
      IS_WITHIN_STATIC_BASE ||
      (CURRENT_PATH === "/" && /\/assets\/js\/main\.js$/i.test(MAIN_SCRIPT_PATH)));
  ...
  function initCustomPlanBuilder() {
    const form = document.querySelector("[data-custom-plan-builder]");
    if (!form) {
      return;
    }

    const cards = Array.from(form.querySelectorAll("[data-bundle-card]"));
    const hiddenInput = form.querySelector("[data-selected-items-json]");
    const summaryList = form.querySelector("[data-bundle-summary-list]");
    const subtotalNode = form.querySelector("[data-bundle-subtotal]");
    const discountNode = form.querySelector("[data-bundle-discount]");
    const taxNode = form.querySelector("[data-bundle-tax]");
    const grandTotalNode = form.querySelector("[data-bundle-grand-total]");
    const taxLabelNode = form.querySelector("[data-bundle-tax-label]");
    const emptyState = form.querySelector("[data-bundle-empty]");
    const countNode = form.querySelector("[data-bundle-selected-info]");
    const billingButtons = Array.from(document.querySelectorAll("[data-billing-toggle] button"));
    const couponCodeInput = form.querySelector("[data-coupon-code]");
    const taxCountryInput = form.querySelector("[data-tax-country]");
    const taxStateInput = form.querySelector("[data-tax-state]");
    const gstNumberInput = form.querySelector("[data-gst-number]");
    const businessPurchaseInput = form.querySelector("[data-is-business-purchase]");

    if (!cards.length || !hiddenInput || !summaryList || !grandTotalNode) {
      return;
    }

    const activeBillingButton = billingButtons.find((button) =>
      button.classList.contains("active")
    );
    let sharedBillingCycle = activeBillingButton?.dataset.billing || "monthly";

    const preselectedKeys = (() => {
      try {
        const raw = form.dataset.preselected || "[]";
        const decoded = JSON.parse(raw);
        if (!Array.isArray(decoded)) {
          return new Set();
        }
        return new Set(decoded.map((item) => String(item)));
      } catch (error) {
        return new Set();
      }
    })();
    ...
    const coupons = (() => {
      try {
        const raw = String(form.dataset.coupons || "[]");
        const parsed = JSON.parse(raw);
    ...
    const taxRules = (() => {
      try {
        const raw = String(form.dataset.taxRules || "[]");
        const parsed = JSON.parse(raw);
        return Array.isArray(parsed) ? parsed : [];
      } catch (error) {
        return [];
    ...
    function readCardPrice(card, cycle) {
      return Math.max(0, Number(card.dataset[cycle] || 0));
    }
    ...
    function findCoupon(code, subtotal) {
      ...
      return (
        coupons.find((coupon) => {
          if (String(coupon.code || "").toUpperCase() !== code) {
            return false;
          }
          if (Number(coupon.is_active || 0) !== 1) {
            return false;
          }
          const minOrder = Number(coupon.min_order_usd || 0);
    ...
    function calculateDiscount(subtotal, coupon) {
      ...
      if (type === "fixed") {
        ...
        discount = Math.round((subtotal * Math.max(0, value)) / 100);
      }

      const maxDiscount = Number(coupon.max_discount_usd || 0);
      if (maxDiscount > 0) {
    ...
    function findTaxRule(countryCode, stateCode) {
      ...
      taxRules.forEach((rule) => {
        if (Number(rule.is_active || 0) !== 1) {
          return;
        }
        const ruleCountry = normalizeCountryCode(rule.country_code || "");
        const ruleState = normalizeStateCode(rule.state_code || "");
        const countryMatches =
          ruleCountry === "*" || ruleCountry === "" || ruleCountry === normalizedCountry;
        const stateMatches = ruleState === "*" || ruleState === "" || ruleState === normalizedState;
        ...
        let score = 0;
        if (ruleCountry === normalizedCountry && normalizedCountry) {
          score += 10;
        } else {
          score += 2;
        }
        if (ruleState === normalizedState && normalizedState) {
          score += 5;
        } else {
          score += 1;
        }
    ...
    function createSummaryRow(itemName, cycle, priceUsd) {
      ...
      cycleText.textContent = billingCycleLabel(cycle);
      left.append(name, cycleText);

      const right = document.createElement("strong");
      right.textContent = formatUsd(priceUsd);
    ...
    function getPayload() {
      ...
      cards.forEach((card) => {
        const checkbox = card.querySelector("[data-bundle-check]");
        const cycleSelect = card.querySelector("[data-bundle-cycle]");
        if (!checkbox || !cycleSelect || !checkbox.checked) {
          return;
        }

        const itemType = String(card.dataset.itemType || "").trim();
        const itemSlug = String(card.dataset.itemSlug || "").trim();
        const itemName = String(card.dataset.itemName || "").trim();
        const billingCycle = String(cycleSelect.value || "monthly").toLowerCase();
        const priceUsd = readCardPrice(card, billingCycle);
    ...
    function renderSummary() {
      ...
      payload.forEach((item) => {
        summaryList.appendChild(
          createSummaryRow(item.item_name || item.item_slug, item.billing_cycle, item.price_usd)
        );
        subtotal += Number(item.price_usd || 0);
      });

      const couponCode = normalizeCouponCode(couponCodeInput?.value || "");
      const coupon = findCoupon(couponCode, subtotal);
      const discount = calculateDiscount(subtotal, coupon);
      const taxable = Math.max(0, subtotal - discount);

      const countryCode = normalizeCountryCode(taxCountryInput?.value || "");
      const stateCode = normalizeStateCode(taxStateInput?.value || "");
      const gstNumber = String(gstNumberInput?.value || "").trim();
      const isBusinessPurchase = Boolean(businessPurchaseInput?.checked);
      const taxRule = findTaxRule(countryCode, stateCode);

      let taxPercent = Number(taxRule?.rate_percent || 0);
      const taxType = String(taxRule?.tax_type || "tax");
      const allowsReverseCharge = Number(taxRule?.reverse_charge_on_gst || 0) === 1;
      if (isBusinessPurchase && gstNumber && taxType === "gst" && allowsReverseCharge) {
        taxPercent = 0;
      }

      const tax = Math.max(0, Math.round((taxable * Math.max(0, taxPercent)) / 100));
      const grandTotal = Math.max(0, taxable + tax);

      hiddenInput.value = JSON.stringify(payload);
      if (subtotalNode) {
        subtotalNode.textContent = formatUsd(subtotal);
      }
      if (discountNode) {
        discountNode.textContent = discount > 0 ? `-${formatUsd(discount)}` : formatUsd(0);
      }
      if (taxNode) {
        taxNode.textContent = formatUsd(tax);
      }
      if (taxLabelNode) {
        const label = taxPercent > 0 ? `${taxType.toUpperCase()} (${taxPercent.toFixed(2)}%)` : "Tax";
        taxLabelNode.textContent = label;
      }
      grandTotalNode.textContent = formatUsd(grandTotal);

      if (emptyState) {
        emptyState.hidden = payload.length > 0;
      }
      if (countNode) {
        countNode.textContent = `${payload.length} item${payload.length === 1 ? "" : "s"} selected`;
      }
    ...
    cards.forEach((card) => {
      ...
      if (!checkbox || !cycleSelect) {
        return;
      }

      if (preselectedKeys.has(itemKey)) {
        checkbox.checked = true;
      }
      cycleSelect.value = cycleSelect.value || sharedBillingCycle;
      cycleSelect.disabled = !checkbox.checked;

      checkbox.addEventListener("change", () => {
        cycleSelect.disabled = !checkbox.checked;
        if (checkbox.checked && !cycleSelect.value) {
          cycleSelect.value = sharedBillingCycle;
        }
        renderSummary();
      });
    ...
    billingButtons.forEach((button) => {
      button.addEventListener("click", () => {
        const cycle = button.dataset.billing || "monthly";
        sharedBillingCycle = cycle;
    ...
    form.addEventListener("submit", (event) => {
      ...
      event.preventDefault();
      const firstCheckbox = form.querySelector("[data-bundle-check]");
      firstCheckbox?.focus();
      window.alert("Select at least one course or tool for your custom plan.");
    });

    [couponCodeInput, taxCountryInput, taxStateInput, gstNumberInput, businessPurchaseInput].forEach(
      (field) => {
        if (!field) {
          return;
        }
  ...
  function initSearchShortcut() {
    ...
    input.addEventListener("keydown", (event) => {
      ...
      if (!query) {
        navigateTo("/ai-tools.php");
      } else {
        navigateTo(`/ai-tools.php?query=${encodeURIComponent(query)}`);
      }
    });
  }
